...
Consistent segmentation accuracy improvement from iteratively adding data from each data engine phase (1, 2 & 3) for both SA-V val set and 9 zero-shot benchmarks is observed. 
{% endcapture %}
{% include callout.html type="note" title="Analysis Results of Data Engine" content=c %}
...
{% capture c %}
SAM 2 **performs well** in accuracy for video segmentation based on first-frame ground-truth mask prompts. SAM 2 performs **significantly better** on SA-V val/test.
{% endcapture %}
{% include callout.html type="note" title="Comparison to SOTA Methods" content=c %}
...
<div class="text-center" style="font-family: monospace; margin: 1.5rem 0; padding: 1rem; background: #f8f9fa; border-radius: 4px;">
FIFO Queue → Eviction Policy → Attention → Tracking Outcome<br>
<span style="color: #666; font-size: 0.9em;">(time-based) &nbsp;&nbsp;&nbsp;&nbsp; (fixed heuristic) &nbsp;&nbsp;&nbsp;&nbsp; (position-invariant) &nbsp;&nbsp;&nbsp;&nbsp; ✓ Success / ✗ Failure</span>
</div>
...
{% endcapture %}
{% include callout.html type="tip" title="Defending the Argument" content=c %}
