...
<small class="text-muted d-block text-center">**Figure 2:** The SAM 2 architecture.</small>
...
The most striking aspect of SAM 2 isn't its segmentation accuracy or its promptable interface -- it's the **memory bank**. At first glance, it reads like a straightforward temporal extension: store past frames, attend to them, maintain identity. But digging into the architecture reveals a subtle tension that sits at the heart of modern video understanding: *how do we remember what matters, without remembering everything?*

SAM 2 maintains object identity across frames by storing two kinds of information in its memory bank:
...
{% capture c %}
**Key distinction**: RNNs encode history into a single vector (making long-range dependencies hard). SAM 2's attention mechanism bypasses that bottleneck entirely -- distance becomes irrelevant. But attention solves the *access* problem, not the *retention* problem.
{% endcapture %}
...
{% endcapture %}
{% include callout.html type="danger" title="Defending the Argument" content=c %}
...
The open question I find most compelling:  
*If we replaced FIFO with a content-aware, learnable memory eviction mechanism -- one that retains frames based on tracking confidence, appearance stability, or semantic salience -- could SAM 2 maintain robust identity over arbitrarily long horizons? And at what computational cost does that shift from an engineering system to a model that truly reasons about what to remember in order to reason better later?*

That's the frontier. SAM 2 shows we've mastered *access* to the past. The next step is mastering *retention* of what matters.
